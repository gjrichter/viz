...
(function() {
	...
	ixmaps.pinnacleChart = function(SVGDocument,args){
		...
		if ( rad >=0 ){
			...
			svg.append("path")
				.attr("d", "M0,0 l100,"+(-rad)+" l100,"+(rad)+"")
				.attr("style","fill:url(#"+(__myGradientId+"R")+");stroke:"+szColor+";stroke-width:20;stroke-opacity:0.3;fill-opacity:"+szFillOpacity+";opacity:"+szOpacity+";");
		}
		...
		if ( args.flag.match(/VALUES/) ){
			...
			if ( nFontSize > (args.theme.nValueSizeMin||120) ){
				var szText = (nValue).toFixed(args.theme.nValueDecimals||120) + (args.theme.szUnits||"");
