...
(function () {
	...
	ixmaps.PCM_DPC_COVID_SEQUENCE_MEAN_3_CLIP = function (theme, options) {
		...
		var broker = new Data.Broker()
			...
			.realize(
				function (dataA) {
					...
					columns.shift();
					columns.shift();
					// drop first 7 for diff and  mean 3
					columns.shift();
					columns.shift();
					columns.shift();
					columns.shift();
					...
					var szXaxisA = [];
					var szMonthA = ["J","F","M","A","M","J","J","A","S","O","N","D"];
					for ( var i =0; i<columns.length; i++ ){
						var dA = columns[i].split("/")
						var d = new Date(dA[2]+"/"+dA[1]+"/"+dA[0]);
						if ( d.getDate() == 1 ){
						  szXaxisA.push(String(szMonthA[d.getMonth()]));
						}else{
						  szXaxisA.push(" ");
						}
					}
