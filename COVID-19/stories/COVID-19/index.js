	...
	**/
	...
	ixmaps.htmlgui_onNewTheme = function(szId){ 
		...
		if ( !themeObj.szFlag.match(/NOLEGEND/) ) {
			...
			$(li).append("<div id='themeLegendDiv"+szId.replace(/\./g,'')+"' class='inline-legend'>"+szHtml+"</div>");
	...
	ixmaps.htmlgui_onDrawTheme = function(szId){ 
       ...
       if (themeObj.szDescription) {
            szHtml += "<p class='legend-description' style='margin-bottom:0.5em'>" + themeObj.szDescription + "</p>";
        } 
		
		var legendHeight = window.innerHeight/2;
		if ( ixmaps.legend.makeColorLegendHTML ){
			szHtml += "<div style='max-height:"+legendHeight+"px;overflow:hidden;margin-top:0em;margin-bottom:0em;padding-right:1em'>";
