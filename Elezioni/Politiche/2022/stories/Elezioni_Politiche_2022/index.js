	...
	**/
	...
	ixmaps.htmlgui_onNewTheme = function(szId){
		...
		if ( !themeObj.szFlag.match(/NOLEGEND/) ) {
			...
			for ( i in clickA ){
				if( $("#themeLegendDiv"+i.replace(/\./g,''))[0] ){
					$("#themeLegendDiv"+i.replace(/\./g,'')).remove();
					clickA[i] = null;
				}
			}
	...
	ixmaps.htmlgui_onDrawTheme = function(szId){ 
		...
		szHtml += "<div id='map-legend-footer' style='clear:both'>";
	...
	ixmaps.changeThemeDynamic = function(szThemeId,szParameter,szFactor){
		...
		ixmaps.htmlgui_onItemClick = function(evt,szId){
			...
			if(szId.match(/theme/)){
				...
				ixmaps.loadStoryTool(ixmaps.loadedProject.map.item||'./item.html');
				return true;
			}
			else{
