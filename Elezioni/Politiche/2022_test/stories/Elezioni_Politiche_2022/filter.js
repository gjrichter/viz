...
(function () {
	...
	var __scanValue = function (nValue) {
		...
		if (String(nValue).match(/,/)) {
			nValue = parseFloat(String(nValue).replace(/\./gi, "").replace(/,/gi, "."));
		} else {
			nValue = parseFloat(String(nValue).replace(/ /gi, ""));
		}
		return isNaN(nValue)?nValue:(isFinite(nValue)?nValue:Number.MAX_SAFE_INTEGER);
	};
