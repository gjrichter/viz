...
(function() {
		...
		ixmaps.bike_stations = function(themeObj){
			...
			var myFeed = Data.feed({source:"https://gbfs.citibikenyc.com/gbfs/en/station_information.json",
					   type:"json",
		...
		ixmaps.bike_status = function(themeObj){
			...
			var myFeed = Data.feed({source:"https://gbfs.citibikenyc.com/gbfs/en/station_status.json",
					   type:"json",
					  }).error(function(error){
						ixmaps.error(error);
					  }).load(function(json){
						...
						themeObj.szSnippet = "<p><b>Citi Bike</b> - Station Status</p><hr>";
		...
		ixmaps.htmlgui_onLegendFooter = function(szId,themeObj,themeDefObj) {
			var d = new Date(ixmaps.bikeUTC*1000-8*60*60*1000);
			return "<p>actual status as of:</p><p>"+d.toString().split("GMT")[0]+"</p>";
		};
