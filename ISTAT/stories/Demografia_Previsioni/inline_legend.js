...
(function() {
	...
	ixmaps.legend.makeColorLegendHTML = function(szId,szLegendId,szMode){ 
		...
		if ( 0 && themeObj.szFilter && themeObj.szFilter.length > 5 ){
			...
			for ( var i=0; i<nRows; i++){
				...
				if ( fCountBars ){
					...
					if ( themeObj.szFlag.match(/DOPACITY/) ){
						szHtml += "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
						szHtml += "</span><span onclick='javascript:ixmaps.markThemeClass(null,\""+szId+"\","+sortA[i].color+");event.stopPropagation();return false;' style='background:"+colorA[sortA[i].color]+";opacity:0.55;font-size:0.5em'>"; 
						szHtml += "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
				...
				if ( sortA[i].count ){
					szHtml += "<span class='theme-legend-count' style='float:right'> "+szCount+" "+szUnit+"</span>";
				}else
		...
		if ( (themeObj.partsA.length == 1) && themeObj.szFlag.match(/DOPACITY/) ){
			...
			for ( var row=0; row<nRows; row++){
				...
				for ( var i=0; i<nColors; i++){
					...
					var nCount = Math.min(40/nColors,8);
					var nOpacity = (1/(row+1));
					if (themeObj.szFlag.match(/DOPACITY/)){
						nOpacity = (i+(nColors/10))/nColors;
					}
					szHtml += "<td><a "+((nCount<=1)?"style='margin-right:-0.5px'":"")+" class='legend-color-button' href='#' title='"+szMinMax+" click to see'><span onclick='javascript:ixmaps.markThemeClass(null,\""+szId+"\","+ix+");event.stopPropagation();return false;' style='background:"+colorA[ix]+";opacity:"+nOpacity+"'>";
	...
	ixmaps.htmlgui_onLegendFooter = function(szId,themeObj){ 
		...
		szHtml += 		"<div style='position:relative;margin-top:0.5em;margin-bottom:1em;margin-left:-0.8em; >";
