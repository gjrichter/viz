...
(function() {
	...
	// -----------------------------------------------
	// check distritos on zoom and pan
	// to see if we neew new tree data
	...
	// -----------------------------------------------
	// intercept onZoomAndPan
	// -----------------------------------------------
	
	ixmaps.checkRefreshTheme = function(){
		
		ixmaps.newSelection("distrito","mapbounds","type:square","selection");
	}
	...
	ixmaps.htmlgui_onSelection = function(szId){ 
		...
		if ( selection.highLightList.itemA.length <= 5 ){
			for ( i in selection.highLightList.itemA ){
				var no = selection.highLightList.itemA[i].node.id.split("::")[1];
				no = ((no.length == 1) ? "0":"") + no;
				var file = "trees_madrid_distrito_" + no + ".csv.gz";
				szUrlA.push(
					"https://s3.eu-west-1.amazonaws.com/data.ixmaps.com/Madrid/trees_madrid_distrito/"+file);
			}
		}
