	...
	__setFacetFilter = function (szFilter) {
		...
		for ( a in objThemesA ){
			...
			if (!objTheme.szFlag.match(/FEATURE/) &&
				!objTheme.szFlag.match(/IMAGE/) &&
				 objTheme.coTable &&
			     objTheme.coTable.match(/MADRID_TREES/) ){
                if ( szFilter ){
		...
		for (var i = 0; i < facetsA.length; i++) {
			...
			if (typeof (facetsA[i].min) != "undefined" && 
				...
				if (facetsA[i].data) {
					...
					szHtml += szScale == "LOG"?" <span style='color:#aaa'>[logarithmic]</span>":"";
					szHtml += "</div>";
					// make slider
					// ---------------------------------
            ...
            if (themesA.length > 1){
                for ( i in themesA ){
                    if ( !themesA[i].szFlag.match(/POSITION|FEATURE/) &&
					      themesA[i].szFlag.match(/MADRID_TREES/)
						){
